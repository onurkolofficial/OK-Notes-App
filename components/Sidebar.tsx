...
const Sidebar: React.FC<SidebarProps> = ({ activeCategory, setActiveCategory, noteCount, onSettingsClick, language }) => {
  ...
  return (
    <aside className="w-56 bg-white dark:bg-black sepia:bg-[#f4ecd8] border-r border-slate-100 dark:border-white/5 h-screen sticky top-0 hidden md:flex flex-col p-6 select-none transition-colors duration-300">
      <div className="flex items-center gap-3 mb-10 group cursor-default">
        {/* Yeni Logo Tasarımı */}
        <div className="w-8 h-8 bg-[#1e1b26] rounded-lg flex items-center justify-center shadow-lg shadow-black/10 group-hover:scale-105 transition-transform duration-300">
        ...
        <h1 className="text-sm font-bold text-slate-950 dark:text-white tracking-tight uppercase">{t.appName}</h1>
      </div>

      <nav className="flex-1 space-y-0.5">
        <p className="text-[9px] font-black text-slate-500 dark:text-slate-700 sepia:text-amber-900 uppercase tracking-widest mb-4 pl-2">{t.categories}</p>
        {categories.map((cat) => (
          <button
            key={cat}
            ...
            className={`w-full flex items-center justify-between px-3 py-2 rounded-lg transition-all duration-200 group ${
              activeCategory === cat
                ? 'bg-slate-100 dark:bg-zinc-900 text-slate-950 dark:text-white font-bold'
                : 'text-slate-600 dark:text-slate-600 hover:text-slate-950 dark:hover:text-slate-300 hover:bg-slate-50 dark:hover:bg-zinc-900/40'
            }`}
          >
            ...
            <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded transition-colors ${
              activeCategory === cat 
              ? 'bg-white dark:bg-black text-slate-950 dark:text-white border border-slate-200 dark:border-white/10' 
              : 'text-slate-500 dark:text-slate-800 group-hover:text-slate-700'
            }`}>
              {noteCount[cat] || 0}
            </span>
          </button>
      ...
      <div className="mt-auto space-y-4">
        <button
          ...
          className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-slate-600 dark:text-slate-700 hover:text-slate-950 dark:hover:text-slate-300 transition-all group border border-transparent hover:border-slate-100 dark:hover:border-white/5"
        >
