...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const [loadedNotes, loadedTheme, loadedLang] = await Promise.all([
          dbService.getAllNotes(),
          dbService.getSetting<Theme>('theme', 'light'),
          dbService.getSetting<Language>('language', 'tr')
        ]);
  ...
  return (
    <div className="flex h-screen bg-white dark:bg-black sepia:bg-[#f4ecd8] transition-colors duration-300 animate-in fade-in relative overflow-hidden">
      <Sidebar 
      ...
      <main className="flex-1 px-6 md:px-12 py-10 overflow-y-auto custom-scrollbar safe-top safe-bottom">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 max-w-[1200px] mx-auto">
          <div className="flex justify-between items-start w-full md:w-auto">
            <div>
              <p className="text-[9px] font-black text-slate-500 dark:text-slate-600 sepia:text-amber-900 uppercase tracking-[0.3em] mb-2">{t.workspace}</p>
              <h1 className="text-3xl font-bold text-slate-950 dark:text-white sepia:text-amber-950 tracking-tight">
            ...
            <button 
              ...
              className="md:hidden p-2 text-slate-500 dark:text-slate-700 transition-all bg-slate-100 dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-white/5"
            >
          ...
          <div className="flex items-center gap-4">
            <div className="relative group flex-1 md:flex-none">
              <svg className="absolute left-0 top-1/2 -translate-y-1/2 text-slate-500 dark:text-slate-800 transition-colors group-focus-within:text-slate-950 dark:group-focus-within:text-slate-400" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
              <input 
                type="text" 
                placeholder={t.search}
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="pl-6 pr-4 py-1.5 bg-transparent border-b border-slate-200 dark:border-white/5 w-full md:w-48 outline-none focus:border-slate-950 dark:focus:border-white/40 transition-all text-xs font-bold text-slate-950 dark:text-white"
              />
            </div>
            
            <button 
              onClick={handleAddNote}
              className="bg-slate-900 dark:bg-white dark:text-black hover:opacity-90 text-white px-5 py-2.5 rounded-xl transition-all active:scale-95 flex items-center gap-2 font-bold text-xs shadow-sm"
            >
        ...
        <div className="max-w-[1200px] mx-auto pb-20">
          {filteredNotes.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-32">
              <h3 className="text-sm font-black text-slate-500 dark:text-slate-800 tracking-tight">{t.noNotes}</h3>
              <button onClick={handleAddNote} className="mt-2 text-xs text-slate-950 dark:text-slate-500 font-black hover:underline underline-offset-4">{t.createOne}</button>
            </div>
          )}
        </div>
      </main>
      ...
      {noteToDelete && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          />
          <div className="bg-white dark:bg-zinc-900 sepia:bg-[#f4ecd8] border border-slate-200 dark:border-white/10 sepia:border-amber-900/10 rounded-3xl p-8 max-w-sm w-full shadow-2xl relative z-10 animate-in zoom-in-95 duration-200 text-center">
            ...
            <h3 className="text-lg font-bold text-slate-950 dark:text-white sepia:text-amber-950 mb-3 tracking-tight">
              {t.confirmDelete}
            </h3>
            
            <p className="text-xs text-slate-600 dark:text-slate-400 sepia:text-amber-950/80 mb-8 leading-relaxed font-bold">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button 
                onClick={() => setNoteToDelete(null)}
                className="px-4 py-3 rounded-xl text-xs font-bold text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors border border-slate-200 dark:border-white/5"
              >
                {t.cancel}
              </button>
              <button 
