...
const NoteCard: React.FC<NoteCardProps> = ({ note, onEdit, onDelete, language }) => {
  ...
  return (
    <div 
      onClick={() => onEdit(note)}
      className="group relative bg-white dark:bg-black sepia:bg-amber-50/10 p-4 rounded-xl border border-slate-200 dark:border-white/10 sepia:border-amber-900/10 shadow-[0_2px_8px_-4px_rgba(0,0,0,0.05)] hover:shadow-lg transition-all duration-300 cursor-pointer flex flex-col h-32 overflow-hidden"
    >
      {/* Üst Bilgi: Renk ve Kategori */}
      <div className="flex justify-between items-start mb-2">
        <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-zinc-900/50 py-1 px-2 rounded-lg border border-slate-200/50 dark:border-white/5">
          <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: note.color }} />
          <span className="text-[7px] font-black text-slate-700 dark:text-slate-400 uppercase tracking-wider">
        ...
        <button 
          ...
          className="p-1 text-slate-400 dark:text-slate-800 hover:text-red-500 transition-all transform hover:scale-110"
        >
      ...
      <div className="flex-1 flex items-center">
        <h3 className="text-[13px] font-bold text-slate-950 dark:text-slate-100 sepia:text-amber-950 line-clamp-2 leading-tight tracking-tight">
      ...
      <div className="mt-2 pt-2 border-t border-slate-100 dark:border-white/5 flex items-center justify-between">
        <div className="flex items-center gap-1.5 text-slate-500 dark:text-slate-800">
